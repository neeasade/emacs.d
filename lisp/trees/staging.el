...
(ns/comment
  (defun ns/headline-content (node)
    ;; get the content of a headline as a string
    ;; todo
    )
  ...
  (defun ns/gather-children (collect-fn path node)
    ;; (message (format "looking at: '%s'" (org-ml-to-string node)))
    (if (org-ml-is-type 'item node)
      ...
      (let* ((item-string (first (s-split "\n" (org-ml-to-string node))))
        ...
        (if (> (length (org-ml--item-get-subitems node))
              0)
          ;; nested!
          (-map (-partial 'ns/gather-children collect-fn path)
            (org-ml--item-get-subitems node))
          ;; go forth :)
          (funcall collect-fn path)))
      ;; todo here: if NO item children in the leaf, need to treat correctly
      (-map (-partial 'ns/gather-children collect-fn path)
        (org-ml-get-children node)))
    )

  (let*
    ((org-default-notes-file (~ "test.org"))
      (leaves (ns/get-notes-nodes 'ns/org-leaf-p)))

    ;; (message (format "qwfp: %s"
    ;;            (length
    ;;              (org-ml-get-children
    ;;                (second leaves)
    ;;                ))))

    ;; (org-ml-get-children (second leaves))
    (-map
      (lambda (leaf)
        (ns/gather-children
          (lambda (full-path)
            (let ((path (->> full-path (-drop-last 1) (s-join ".")))
                   (val (last full-path)))
              ;; (message (format "arst: %s = '%s'qwfp" path val))
              (message (format (s-join "." full-path)))
              ;; (format "arst: %s = '%s'" path val)
              )
            )
          (org-ml-headline-get-path leaf)
          leaf))
      ;; (list (second leaves))
      leaves
      )
    ...
    ;;   (lambda (leaf)
    ;;     (let*
    ;;       ((path (s-join "." (org-ml-headline-get-path leaf)))
    ;;         (list-items
    ;;           (->> leaf
    ;;             (org-ml-get-children)
    ;;             (-map (lambda (node)
    ...
    ;;                         first
    ;;                         (org-ml-get-type)
    ;;                         (-contains-p '(paragraph plain-list))
    ;;                         ))))
    ;;             (-filter (fn (org-ml-is-type 'section)))
    ;;             ;; (org-ml-get-children)
    ;;             ;; (-map 'org-ml-get-type)
    ;;             )
    ;;           ;; (-filter (fn (org-ml-is-type 'lisp-item <>)))
    ;;           ))
