...
(ns/iterate-color
  "#000000"
  (lambda (c)
    (message (format "called with %s" c))
...
(defun ns/color-name-distance (c1 c2)
  (color-cie-de2000
    (->> c1 color-name-to-rgb (apply 'color-srgb-to-lab))
    (->> c2 color-name-to-rgb (apply  'color-srgb-to-lab))))

(ns/color-name-distance "#000000" "#ffffff")
(ns/color-name-distance "#ffffff" "#000000" )
...
(base16-theme-define 'neea
  (list
    ;; The comments on the sections here are from the base16 styling guidelines, not necessarily what the emacs package follows.
